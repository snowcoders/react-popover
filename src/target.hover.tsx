...
export class TargetHover extends React.Component<TargetHoverProps, TargetHoverState> {
    ...
    render() {
        return <Target componentFactory={(targetProps) => (
            <span
                {...targetProps}
                className={classnames("sci-react-popover--target", "hover", this.props.className)}
                tabIndex={0}
                onMouseOver={() => { this.onHoverChange(true) }}
                onMouseOut={() => { this.onHoverChange(false) }}
                onFocus={() => { this.onHoverChange(true) }}
                onBlur={() => { this.onHoverChange(false) }}>
                {this.props.children}
            </span>
        )} />;
    }
